...
template <class Te> class NoDefault : public Te
{
    ...
    *(static_cast<EbmlUInteger *>(&MyDocTypeReadVer)) = 2;
    ...
    for (unsigned int i=0;i<Input->MkvGetStreamCount();i++)
    {
        ...
        if (NULL!=ti->lang)
        {
            ...
            cur_track->PushElement(*new NoDefault<KaxTrackLanguage>);
        ...
        if (track_info[i].compression_type!=MKV_TRACK_COMPRESSION_NONE)
        {
            ...
            cc.PushElement(*new NoDefault<KaxContentCompAlgo>);
    ...
    int64_t cluster_timecode=0,frame_timecode;
