...
void MainWnd::AppExpired()
{
    ...
    QMessageBox msgBox(QMessageBox::Warning,UI_QSTRING(APP_CAPTION_MSG),UI_QSTRING(PROT_DEMO_KEY_EXPIRED),QMessageBox::Ok,this);
...
void MainWnd::SlotOpenFiles()
{
    ...
    if (filter.isEmpty())
    {
        filter = QStringFromUtf16(m_app->GetAppString(AP_vastr_OpenFileFilter));
    }
    ...
    m_app->OpenFile(Utf16FromQString(fileName));
}
...
void MainWnd::DoProcessLogMessage(QString Message,unsigned int Flags)
{
    ...
    {
        logtext_buf.append(Qt::escape(Message));
    } else {
    ...
    )
{
        ...
        ( (Flags&AP_UIMSG_BOX_MASK) == AP_UIMSG_BOXWARNING ))
    {
        ...
        QMessageBox msgBox(icon,UI_QSTRING(APP_CAPTION_MSG),QStringFromConstUtf16(Text),QMessageBox::Ok,this);
        msgBox.setEscapeButton(QMessageBox::Ok);
...
void MainWnd::RestoreGeometry()
{
    ...
    if (strValue)
    {
        byteBase = QStringFromConstUtf16(strValue).toLatin1();
