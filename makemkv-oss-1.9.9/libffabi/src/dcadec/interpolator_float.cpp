...
INTERPOLATE_LFE(lfe_float_fir)
{
    ...
    for (int i = 0; i < nsamples; i++) {
        ...
        for (int j = 0; j < dec_factor; j++) {
            ...
            pcm_samples[(i * dec_factor + j) << (synth_x96?1:0)] = convert(res);
        }
    }
...
INTERPOLATE_LFE(lfe_float_iir)
{
    ...
    int dec_factor = 64 << (dec_select?1:0);
    ...
    for (int i = 0; i < nsamples; i++) {
        ...
        for (int j = 0; j < dec_factor; j++) {
            ...
            // Save interpolated samples
            pcm_samples[(i * dec_factor + j) << (synth_x96?1:0)] = convert(res1);
            res1 = 0.0;
        }
    }
...
INTERPOLATE_SUB(sub32_float)
{
    ...
    dbg_assert(subband_samples_hi == NULL);
