                            ...
                            FFM_MatrixEncoding matrix_encoding)
{
    ...
    if (!in_layout || in_channels > FFM_AVRESAMPLE_MAX_CHANNELS)
        return AVERROR(EINVAL);
    if (!out_layout || out_channels > FFM_AVRESAMPLE_MAX_CHANNELS)
        return AVERROR(EINVAL);
    ...
    if (unaccounted & AV_CH_BACK_CENTER) {
        if (out_layout & AV_CH_BACK_LEFT) {
            ...
            if (matrix_encoding == FFM_MATRIX_ENCODING_DOLBY ||
                matrix_encoding == FFM_MATRIX_ENCODING_DPLII) {
    ...
    if (unaccounted & AV_CH_BACK_LEFT) {
        if (out_layout & AV_CH_BACK_CENTER) {
            ...
            } else if (matrix_encoding == FFM_MATRIX_ENCODING_DPLII) {
